...
一个让 Agent 可靠检索数据的 Skill 集。核心不是「能搜到数据」，而是「搜到的数据可信、可追溯、可审计」。当前包含：人口统计、上市公司销售数据两个窄领域 Skill（共用六模块架构，各自替换信源清单和检索策略），以及一个元层次 Skill（可信检索搭建——为任意未预置领域自动构建检索流程）。
...
> 为任意未预置 Skill 的数据领域自动搭建可信检索流程——领域分析前置，信源白名单现场构建，达标即停封槽。

覆盖未预置领域的陌生查询（统计指标、财务数据、经济数据、教育指标等）。树状/网状/单源垄断三种信源结构自适应，储备清单补位，数据/归因信源分离，可审计交付。

→ [SKILL.md](./可信检索搭建/SKILL.md) | [测试实录（整合版）](./可信检索搭建/测试实录.md) | [设计总结](./可信检索搭建/设计总结.md)
...
| **Agent GSR** | 门控补充检索（ReAct 工程化变体） | 直接复用为「补搜机制」，从业务专用管线泛化为通用机制 |
